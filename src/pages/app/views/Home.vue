<template>
  <div class="home">
    <YScroll>
      ...
      <div class="drawer" v-for="(drawer, index) in drawers.filter(d => d.movies && d.movies.length)" :key="index">
      ...
      if (data.fromPos.y > 1) {
        if (data.toPos.y === data.fromPos.y) { // 水平移动
          ...
          if (data.toPos.y > 1 && [keyCodeMap.up, keyCodeMap.down].includes(data.keyCode)) {
            const computedX = data.fromPos.x - this._hMoved
            if (computedX < data.toPos.x) {
              data.toPos.x = computedX
            }
          }
          this._hMoved = 0
