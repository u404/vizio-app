...
export default {
  ...
  computed: {
    ...mapState(["tempFilmSku", "tempFilmDetail"]),
    buyUrl () {
  ...
  methods: {
    ...
    autoGetActivityBuyStatus (enabled = true) {
      if (this._autoRunGetActivityBuyStatus) {
        this._autoRunGetActivityBuyStatus.cancel()
        this._autoRunGetActivityBuyStatus = null
      }

      if (enabled) {
        const fn = () => services.getActivityInfo(this.actId).catch(() => null).then(res => {
          if (res && res.packBuyStatus) {
            this.paySuccessHandler()
            throw new Error()
          }
        })
        this._autoRunGetActivityBuyStatus = new AutoRun(fn, 1000)
      }
    },
    ...
    paySuccessHandler () {
      ...
      this.showPaying()
      this.autoGetActivityBuyStatus(false)
    },

    report (event) {
      this.$reporter.track(event, { sku_id: this.id, film_name: this.tempFilmDetail && decodeURIComponent(this.tempFilmDetail.filmName) })
    },
  ...
  async mounted () {
    ...
    this.autoGetActivityBuyStatus(true)
